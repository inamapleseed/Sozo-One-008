...
class Cart {
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$product_special_query2 = $this->db->query("SELECT isbn FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$cart['product_id'] . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00' OR date_start <= CURDATE()) AND (date_end = '0000-00-00' OR date_end >= CURDATE())) ORDER BY priority ASC, price ASC LIMIT 1");

				$product_special_query3 = $this->db->query("SELECT mpn FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$cart['product_id'] . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00' OR date_start <= CURDATE()) AND (date_end = '0000-00-00' OR date_end >= CURDATE())) ORDER BY priority ASC, price ASC LIMIT 1");
				...
				// << Related Options / Связанные опции 
					...
					&& isset($ro_settings['spec_price_special']) && $ro_settings['spec_price_special'] ) {
						...
						//foreach ($ro_for_products[$key] as $ro_comb) {
							...
							if ($ro_comb['specials']) {
								$product_ro_special_query = $this->db->query("SELECT price FROM ".DB_PREFIX."relatedoptions_special WHERE relatedoptions_id = '" . (int)$ro_comb['relatedoptions_id'] . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' ORDER BY priority ASC, price ASC LIMIT 1");
				...
				// >> Related Options / Связанные опции
				$cart_totality = $this->countProducts2();

					$price = $product_query->row['price'];
					$priceTwo = $product_query->row['isbn'];
					$priceThree = $product_query->row['mpn'];
					
				if ($product_special_query->num_rows) {
					if (empty($special_status)) {
						$price = $product_special_query->row['price'];
						$price2 = $product_special_query2->row['isbn'];
						$price3 = $product_special_query3->row['mpn'];
						// << Related Options / Связанные опции 
			
							if ( !empty($ro_price_data['price_modificator']) ) {
								$price = $price + $ro_price_data['price_modificator'];
								$price2 = $price2 + $ro_price_data['price_modificator'];
								$price3 = $price3 + $ro_price_data['price_modificator'];
							}
						// >> Related Options / Связанные опции
					}
					foreach ($cart_query->rows as $cart_2) {
						if ($cart_totality <= 1 || $cart_totality == 0) {
							$price = $product_special_query->row['price'];
						} else if ($cart_totality  == 2) {
							$price = $product_special_query2->row['isbn'];
						} else {
							$price = $product_special_query3->row['mpn'];
						}
					}					
				} else {
				...
				$product_data[] = array(
					...
					'isbn'           => ($priceTwo + $option_price),
					'mpn'           => ($priceThree + $option_price),
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				// << Related Options / Связанные опции 
					...
					&& isset($ro_settings['spec_price_discount']) && $ro_settings['spec_price_discount'] ) {
						...
						//foreach ($ro_for_product as $ro_comb) {
							...
							if ($ro_comb['discounts']) {
								$ro_discount_quantity = $ro_quantities[$ro_comb['relatedoptions_id']];
								$product_ro_discount_query = $this->db->query("SELECT price FROM " . DB_PREFIX . "relatedoptions_discount WHERE relatedoptions_id = '" . (int)$ro_comb['relatedoptions_id'] . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND quantity <= '" . (int)$ro_discount_quantity . "' ORDER BY quantity DESC, priority ASC, price ASC LIMIT 1");
								if ($product_ro_discount_query->num_rows) {
									$product_discount_query = $product_ro_discount_query;
									break;
								}
				...
				// << Related Options / Связанные опции 
					...
					&& isset($ro_settings['spec_price_special']) && $ro_settings['spec_price_special'] ) {
						...
						//foreach ($ro_for_products[$key] as $ro_comb) {
							...
							if ($ro_comb['specials']) {
								$product_ro_special_query = $this->db->query("SELECT price FROM ".DB_PREFIX."relatedoptions_special WHERE relatedoptions_id = '" . (int)$ro_comb['relatedoptions_id'] . "' ORDER BY priority ASC, price ASC LIMIT 1");
								if ($product_ro_special_query->num_rows) {
									$product_special_query = $product_ro_special_query;
									break;
								}
				...
				if ($product_special_query->num_rows) {
					if (empty($special_status)) {
						$price = $product_special_query->row['price'];
						$priceTwo = $product_special_query2->row['isbn'];
						$priceThree = $product_special_query3->row['mpn'];

						// << Related Options / Связанные опции 
			
							if ( !empty($ro_price_data['price_modificator']) ) {
								$price = $price + $ro_price_data['price_modificator'];
								$priceTwo = $priceTwo + $ro_price_data['price_modificator'];
								$priceThree = $priceThree + $ro_price_data['price_modificator'];
							}
						// >> Related Options / Связанные опции
					}

					foreach ($cart_query->rows as $cart_2) {
						if ($cart_2['quantity'] <= 1) {
							$price = $product_special_query->row['price'];
						} else if ($cart_2['quantity'] == 2) {
							$price = $product_special_query2->row['isbn'];
						} else {
							$price = $product_special_query3->row['mpn'];
						}
	...
	public function countProducts2() {
		$product_total = 0;

		$products = $this->getProducts2();
