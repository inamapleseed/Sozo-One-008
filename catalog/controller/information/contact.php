<?php
	class ControllerInformationContact extends Controller {
		...
		$data['urlServer'] = $server;
			...
			$mail->setSubject(html_entity_decode(sprintf($this->language->get($this->request->post['from_name']), $this->request->post['from_name']), ENT_QUOTES, 'UTF-8'));
		...
		protected function validate_enquiry() {
			...
			if ((utf8_strlen($this->request->post['ref_no']) < 3) || (utf8_strlen($this->request->post['ref_no']) > 32)) {
			$this->error['ref_no'] = $this->language->get('error_ref_no');
			}

			if ((utf8_strlen($this->request->post['from_name']) < 2) || (utf8_strlen($this->request->post['from_name']) > 32)) {
