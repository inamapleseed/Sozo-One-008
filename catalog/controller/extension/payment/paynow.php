...
class ControllerExtensionPaymentPaynow extends Controller {
	public function index() {
		...
		$data['action'] = $this->url->link('extension/payment/paynow');
		
		$data['success_ref'] = false;
		
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
		    $this->load->model('checkout/order');
		    
		    $file = '';
			$uploads_dir = 'image/catalog/receipt/'; // set you upload path here
			if (is_uploaded_file($this->request->files['receipt']['tmp_name'])) {
			    
			   move_uploaded_file($this->request->files['receipt']['tmp_name'], $uploads_dir.$this->request->files['receipt']['name']);

			  $file = $this->request->files['receipt']['name'];

			}
		
	
		    $this->model_checkout_order->addRef($this->request->post['ref_no'],$uploads_dir.$file, $this->session->data['latest_order']);
		    
		    $data['success_ref'] = 'You successfully submitted your Reference Number and Receipt';
