...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		$data['text_desc_content'] = $this->language->get('text_desc_content');
		...
		if ($product_info) {
			...
			$this->document->addStyle('catalog/view/javascript/magnifyjs/magnify.css');
			$this->document->addScript('catalog/view/javascript/magnifyjs/jquery.magnify.js');
			$this->document->addScript('catalog/view/javascript/magnifyjs/jquery.magnify-mobile.js');
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
                if ($this->config->get('config_product_decimal_places')) {
					...
					$data['isbn'] = $this->currency->format($this->tax->calculate($product_info['isbn'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
			...
			if ((float)$product_info['special']) {
                if ($this->config->get('config_product_decimal_places')) {
					...
					$data['special2'] = $this->currency->format($this->tax->calculate($product_info['special2'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
					$data['special3'] = $this->currency->format($this->tax->calculate($product_info['special3'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                } else {
					...
					$data['special2'] = $this->currency->format2($this->tax->calculate($product_info['special2'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
					$data['special3'] = $this->currency->format2($this->tax->calculate($product_info['special3'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                }
			} 
			...
			if ($this->config->get('config_tax')) {
				...
				$data['tax'] = $this->currency->format((float)$product_info['special2'] ? $product_info['special2'] : $product_info['price'], $this->session->data['currency']);
				$data['tax'] = $this->currency->format((float)$product_info['special3'] ? $product_info['special3'] : $product_info['price'], $this->session->data['currency']);
			} 
			...
			foreach ($discounts as $discount) {
                if ($this->config->get('config_product_decimal_places')) {
					$data['discounts'][] = array(
						...
						'isbn'    => $this->currency->format($this->tax->calculate($discount['isbn'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']),
						'mpn'    => $this->currency->format($this->tax->calculate($discount['mpn'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'])
					);
                } else {
					$data['discounts'][] = array(
						'quantity' => $discount['quantity'],
						...
						'isbn'    => $this->currency->format2($this->tax->calculate($discount['isbn'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']),
						'mpn'    => $this->currency->format2($this->tax->calculate($discount['mpn'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'])
					);
                }
			}
					...
					if (!$option_value['subtract'] || ($option_value['quantity'] >= 0)) {
						if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
			                if ($this->config->get('config_product_decimal_places')) {
								...
								$isbn = $this->currency->format($this->tax->calculate($option_value['isbn'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);

								$mpn = $this->currency->format($this->tax->calculate($option_value['mpn'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
			                } else {
								...
								$isbn = $this->currency->format2($this->tax->calculate($option_value['isbn'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);

								$mpn = $this->currency->format2($this->tax->calculate($option_value['mpn'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
			                }
						} else {
							$price = false;
							$isbn = false;
...
// debug($results);
	...
	public function updatePrice() {
		...
		if ($product_info['special']) {
			...
			$isbn = $product_info['special'];
			$mpn = $product_info['special'];
			$new_price_found = 1;
		}
		
		$total_price = $price + $option_price;
		$total_price2 = $isbn + $option_price;
		$total_price3 = $mpn + $option_price;
		
		// Total Calculation
		$unit_price = $this->tax->calculate($total_price, $product_info['tax_class_id'], $this->config->get('config_tax'));
		$unit_price2 = $this->tax->calculate($total_price2, $product_info['tax_class_id'], $this->config->get('config_tax'));
		$unit_price3 = $this->tax->calculate($total_price3, $product_info['tax_class_id'], $this->config->get('config_tax'));

        if ($this->config->get('config_product_decimal_places')) {
			$total = $this->currency->format($unit_price * $this->request->post['quantity'], $this->session->data['currency']);
			$total2 = $this->currency->format($unit_price2 * $this->request->post['quantity'], $this->session->data['currency']);
			$total3 = $this->currency->format($unit_price3 * $this->request->post['quantity'], $this->session->data['currency']);
        } else {
			$total = $this->currency->format2($unit_price * $this->request->post['quantity'], $this->session->data['currency']);
			$total2 = $this->currency->format2($unit_price2 * $this->request->post['quantity'], $this->session->data['currency']);
			$total3 = $this->currency->format2($unit_price3 * $this->request->post['quantity'], $this->session->data['currency']);
        }
