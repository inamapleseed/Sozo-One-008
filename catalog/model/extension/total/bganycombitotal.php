...
class ModelExtensiontotalbganycombitotal extends Controller {	
	...
	public function __construct($registry) {
 	...
 	private function ghost($quantity = 0,$buyqty = 0) {
		$return = $quantity;
			...
			if($buy_product && $get_product) {
				...
				foreach($get_product as $getproduct) {
					...
					if(isset($buy_product[$bganycombi_id]) && count($buy_product[$bganycombi_id]['products']) >= (int)$info['buyqty']) {
						...
						/* 
							ghost - fixes for buy 3 get 2 free 
							issue: buy 5 will get 3 free > Correction : buy 6 then free 4, buy 5 only free 2
						*/
						$getfreeqty = floor(($this->ghost(count($buy_product[$bganycombi_id]['products']),$info['buyqty']) / $info['buyqty']) * $info['getqty']);
